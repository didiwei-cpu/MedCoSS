...
def build_loader(args, split):
    ...
    if split == 'train':
        counts = np.bincount(ds.label_list, minlength=2).astype(np.float32)
        class_weights = counts.sum() / (len(counts) * counts)
        sample_weights = [class_weights[l] for l in ds.label_list]
        sampler = torch.utils.data.WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True)
        loader = DataLoader(ds, batch_size=args.batch_size, sampler=sampler,
                            num_workers=args.num_workers, pin_memory=True)
        return loader, crop, torch.tensor(class_weights, dtype=torch.float32)
    else:
...
def main():
    ...
    train_loader, crop, class_weights = build_loader(args, 'train')
